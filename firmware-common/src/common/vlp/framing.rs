...
impl Packet {
    pub fn serialize(&self) -> Vec<u8, MAX_PAYLOAD_LENGTH> {
    ...
    pub fn deserialize(mut packet: Vec<u8, MAX_PAYLOAD_LENGTH>) -> Result<Packet, FramingError> {
        if packet.len() < 3 {
...
pub enum FramingError {
    ...
    MalformedPacket(#[defmt(Debug2Format)] Vec<u8, MAX_PAYLOAD_LENGTH>),
}
