...
// TODO use embassy sync once they check will_wake in multi waker registration
...
impl<const N: usize> MultiWakerRegistration<N> {
    ...
    pub fn register<'a>(&mut self, w: &'a Waker) -> Result<(), &'a Waker> {
        let already_registered = self
            ...
            .iter_mut()
            .find(|waker_slot| {
                if let Some(existing_waker) = &waker_slot.waker {
                    return existing_waker.will_wake(w);
                }
                return false;
            })
            .is_some();
