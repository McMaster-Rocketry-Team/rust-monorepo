...
pub trait AsyncEraseFlash {
    ...
    async fn erase_sector_4kib_nb(&mut self, address: u32) -> Result<(), Self::Error>;
    async fn erase_block_32kib_nb(&mut self, address: u32) -> Result<(), Self::Error>;
    async fn erase_block_64kib_nb(&mut self, address: u32) -> Result<(), Self::Error>;

    async fn suspend_erase(&mut self) -> Result<(), Self::Error>;
    async fn resume_erase(&mut self) -> Result<(), Self::Error>;
    async fn is_busy(&mut self) -> Result<bool, Self::Error>;
    async fn wait_erase_finish(&mut self) -> Result<(), Self::Error>;
